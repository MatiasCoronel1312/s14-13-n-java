...
const RedDeAgencias = () => {
  ...
  const handleReserve = (id: number) => {
    let agencia = allAgencias.find((obj) => obj.id === id);
    dispatch(postReserve({ lugar: agencia?.name }));
    window.scrollTo({
      top: 0,
      behavior: "smooth",
    });
  ...
  return (
    <div className="w-full bg-background flex justify-center">
      <div className="flex items-center flex-col lg:w-[85%] md:w-[90%] ">
        ...
        <div className="text-text text-[22px] text-left w-full font-semibold mb-3">
          Encontrá la agencia más cercana:
        </div>
        <div className="w-full rounded-[10px] my-10">
          {allAgencias.map((agencia, index) => (
            <div
              key={agencia.id}
              className={`text-text w-full h-28 bg-[#${
                index % 2 === 0 ? "F8C381" : "background"
              }] flex justify-between`}
            >
              <div className="flex justify-center items-center h-full w-28 ">
                {airplane(agencia.name)}
              </div>
              ...
              <div className="h-full flex flex-col justify-evenly w-[30%]">
                ...
                <div>
                  <button
                    onClick={() => {
                      handleReserve(agencia.id);
                    }}
                    className="Gradient-H_hover border-2 border-accent text-accent hover:text-text hover:border-none w-3/4 h-10 rounded-lg "
                  >
                    Reservar
                  </button>
                </div>
